...
const imageUrls = [
    'https://i.pinimg.com/736x/c8/6f/45/c86f453be5733d28c651a8dfd6ae38b3.jpg',
    'https://i.pinimg.com/736x/69/2c/70/692c70ba149d94fdb213fde972b0381b.jpg',
    'https://i.pinimg.com/736x/2b/2d/20/2b2d20c4f2758036d9bad22be9eb123a.jpg',
    'https://i.pinimg.com/originals/0f/17/a1/0f17a1a9da6a6e3c143a141c22d1be16.gif',
    'https://i.pinimg.com/originals/bf/fa/eb/bffaeb0bcd7fcdfd7e3ae9bfe8acd7a9.gif',
...
//         // Set the src attribute of the img element to the random URL
//         document.getElementById('randomImage').src = randomUrl;
//     } else {
//         alert('No image URLs available.');
//     }
// }

// // Define the sentences with multiple lines
// const sentences = `When The Going Gets Tough.
// the tough gets
// G O I N G`;

// // Function to add random spacing
// function addRandomSpacing(text) {
//     return text.split('').map(char => {
//         if (char === ' ') {
//             return ' '.repeat(Math.floor(Math.random() * 5) + 1);
//         } else {
//             return char + ' '.repeat(Math.floor(Math.random() * 5));
//         }
//     }).join('');
// }

// // Function to display the sentences with random spacing
// function displayRandomSpacing() {
//     const lines = sentences.split('\n');
//     const randomLines = lines.map(line => addRandomSpacing(line)).join('\n');
//     document.getElementById('randomSentence').textContent = randomLines;
// }

// // Initial display of the sentences with random spacing on page load
// window.onload = displayRandomSpacing;




// // Define the sentences with multiple lines
// const sentences = `This is the first sentence.
// This is the second sentence.
// This is the third sentence, continuing on a new line.`;

// // Function to add random spacing and color
// function addRandomAttributes(text) {
//     return text.split('').map(char => {
//         const randomSpacing = ' '.repeat(Math.floor(Math.random() * 5) + 1);
//         const randomColor = '#' + Math.floor(Math.random()*16777215).toString(16); // Random hexadecimal color
//         return `<span style="color:${randomColor};">${char}</span>${randomSpacing}`;
//     }).join('');
// }

// // Function to display the sentences with random spacing and color
// function displayRandomAttributes() {
//     const lines = sentences.split('\n');
//     const randomLines = lines.map(line => addRandomAttributes(line)).join('\n');
//     document.getElementById('randomSentence').innerHTML = randomLines;
// }

// // Initial display of the sentences with random spacing and color on page load
// window.onload = displayRandomAttributes;




// Define the sentence with multiple lines
const sentences = `When the going 
gets 
t o u g h     , 
           the 
tough get 
...
// Function to add random spacing and color to text
function addRandomAttributes(text) {
    return text.split('').map(char => {
        const randomSpacing = ' '.repeat(Math.floor(Math.random() * 5) + 1);
        const randomColor = '#' + Math.floor(Math.random()*16777215).toString(16); // Random hexadecimal color
...
// Function to display random image and text
function displayRandomContent() {
    ...
    if (imageUrls.length > 0) {
        ...
        document.getElementById('randomImage').src = ''; // Clear image if no URLs are available
    }

    // Display random text with spacing and color
    const lines = sentences.split('\n');
    const randomLines = lines.map(line => addRandomAttributes(line)).join('\n');
    document.getElementById('randomSentence').innerHTML = randomLines;
}

// Initial display of random image and text on page load
